...
export default function ProfileRefuse({ user }: { user: any }) {
    const { sportsMap, loading, error } = useManagedAreas(user.id)
    const sportsArray = Object.values(sportsMap || {});
    if (error) return <Alert variant={"danger"} className="text-start">
        {String(error)}
    </Alert>;
    return <div>
        {loading ? <div className="text-center">
            ...
            <div className="d-flex justify-content-between align-items-center">
                <h3 className="fs-3 fw-bold">Hồ sơ:  {user !== null && user.nameStore !== "" ? user.nameStore : <span className="text-small text-secondary">chưa xác định</span>}</h3>
            </div>
                ...
                <div className="m-0 p-0">
                    ...
                    <div className="m-0 p-3 py-2 rounded-top-4 bg-header text-light d-flex justify-content-between px-3 align-items-center">
                        <p className="fw-bold fs-4 align-items-center m-0 p-0 ">
                            {user && user.name !== "" ? user.name : <span className="text-small text-secondary">Chưa xác định</span>}
                        </p>
                    ...
                    <div className=" m-0 row border border-black rounded-bottom-4 ">
                        <div className=" col-12 col-md-6 p-0  ">
                            ...
                            <div className="profileUser border-bottom border-black px-2 ">
                                <div>
                                    <p className=" fw-bold p-0 my-1">Họ và tên: {user && user.name !== "" ? user.name : <span className="text-small text-secondary">chưa xác định</span>}</p>
                                    <p className=" fw-bold p-0 my-1">Ngày sinh: {user && user.dob !== null ? <FormatDate timestamp={user.dob} /> : <span className="text-small text-secondary">.. / .. /....</span>}</p>
                                    <p className=" fw-bold p-0 my-1">Giới tính: {user && user.gender !== null ? user.gender : <span className="text-small text-secondary">chưa xác định</span>}</p>
                                    <p className=" fw-bold p-0 my-1">Địa chỉ: {user && user.address !== "" ? user.address : <span className="text-small text-secondary">chưa xác định</span>}</p>
                                    <p className=" fw-bold p-0 my-1">Số điện thoại: {user && user.name !== "" ? user.phone : <span className="text-small text-secondary">chưa xác định</span>}</p>
                                </div>
                            ...
                            <div className="profileFields border-bottom border-black px-2 d-block d-md-none">
                                <div>
                                    <p className="fw-bold p-0 my-1">Tên trụ sở: {user ? user.nameStore : <span className="text-small text-secondary">chưa xác định</span>}</p>
                                    <p className="fw-bold p-0 my-1">Mô hình hoạt động: {sportsArray.length > 0 ? sportsArray.map((item: any, key: number) => (
                                        ...
                                        :
                                        <span className="text-small text-secondary">Chưa có danh mục</span>}
                                    </p>
                            ...
                            <div className="infoCard px-2">
                                ...
                                {user ? <div className="row m-0 p-0">
                                    ...
                                    <div className="m-0 p-2 ps-0 col-12 col-md-6">
                                        ...
                                        <img src={user.documentInfo.card_back !== "" && user.length > 0 ? user.documentInfo.card_back : "http://media.vov.vn/sites/default/files/styles/large/public/2021-10/Can%20cuoc.jpg"} className="rounded float-start w-100 m-1 ms-0" alt="..."></img>
                                        {/*  */}
                                    </div>
                            ...
                            <div className="busniessLicense px-2 d-block d-md-none border-top border-black">
                                ...
                                <div className="m-2 d-flex justify-content-center">
                                    <img src={user.documentInfo.business_license_image !== "" && user.length > 0 ? user.documentInfo.business_license_image : "https://ketoanlacviet.vn/wp-content/uploads/2024/08/mau-giay-chung-nhan-dang-ky-doanh-nghiep-2024.png"} className="rounded float-start w-75 m-1 ms-0" alt="..."></img>
                                </div>
                            </div>
